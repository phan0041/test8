...
def home_add(request):
    ...
    if request.user.is_authenticated():
        param['user']='logined'
    return render_to_response('index.html',param)
def home_ele(request):
    param={}
    if request.user.is_authenticated():
        param['user']='logined'
    return render_to_response('eindex.html',param)
def login_process(request):
##    user = User.objects.create_user('phanvandan', 'lennon@thebeatles.com', 'phanvandan')
##    user = User.objects.create_user('schui', 'lennon@thebeatles.com', 'schui')
##    user = User.objects.create_user('huisiucheung', 'lennon@thebeatles.com', 'huisiucheung')
...
def AMaths_Menu(request,subj_id):
	...
	dan=paperset.objects.filter(subject_id=subj_id)
	param['papersets']=dan
	last = len(dan)-1
	param['last']=dan[last]
...
def add_math_question(request,list_type,subj_id,page_no):
	...
	if (request.GET.get("paperset_id_start") != None):
		paperset_id_start = int(request.GET.get("paperset_id_start"))
	if (request.GET.get("paperset_id_end") != None):
		paperset_id_end = int(request.GET.get("paperset_id_end"))
...
##			sel = list(question.objects.filter(topic_id=list_id).only('id','content','question_no','marks').order_by('id').values())
                ...
                papersets = paperset.objects.filter(id__gte=paperset_id_start,id__lte=paperset_id_end)
                dan1 = paperset_id_start
                paper_ids = []
                while (dan1 <= paperset_id_end):
                        paper_id = paper.objects.filter(paperset_id = dan1).values('id')
                        paper_ids.append(paper_id)
                        dan1 = dan1 +1
                param['dan2']=paper_ids
                paper_ids = paper.objects.filter(paperset_id__in=papersets).values('id')
                sel = list(question.objects.filter(topic_id=list_id,paper_id__in=paper_ids).only('id','content','question_no','marks').order_by('id').values())
	...
	#for links
	param['subject']=subject.objects.all()
	param['papersets']=paperset.objects.filter(subject_id=subj_id)
	if request.user.is_authenticated():
            param['user']='logined'
...
def add_math_concept(request,subj_id):
	param={}
	#for topic dropdown
	param['topics'] = topic.objects.filter(subject_id_id=subj_id).order_by('idorder')
	...
	if request.GET.get('tag_type')!=None:
                tag_type = request.GET.get('tag_type')
                param['tag_type'] = tag_type
	if tpc_id != None and tag_type != None: #if topic_id found, display the concepts and formulae only
		taglist = tag_definitions.objects.filter(topic_id=tpc_id, type__in='C').order_by('idorder')
...
def add_math_formula(request,subj_id):
	param={}
	#for topic dropdown
	topics = topic.objects.filter(subject_id_id=subj_id).order_by('idorder')
	tag_type = 'F'
        ...
        for t in topics:
                item = {}
                item['title'] = t.title
                formula = tag_definitions.objects.filter(topic_id = t.id,type__in=[tag_type])
                item['formula']=  formula
                if ((t.id != 13) and (len(formula) > 0)):
                        display.append(item)
	param['display']=display
	param['subj_id']=subj_id
	if request.user.is_authenticated():
            param['user']='logined'
	return render_to_response('add_math_formula.html',param)
...
def analyzer_paper_tag(request,subj_id):
	...
	if request.GET.get("paperset_id") != None: #compute only when a paperset is selected
		...
		paperset_id_end = request.GET.get("paperset_id_end")
		topic_id = request.GET.get("topic_id")
		combi = int(request.GET.get("combi"))
		tag_type = request.GET.get("tag_type")
		if (topic_id == "All topics"):
                        topics = topic.objects.filter(subject_id = subj_id).values('id')
                else:
                        topics = [topic_id]
		#retrieve the questions
                papersets = paperset.objects.filter(id__gte=paperset_id,id__lte=paperset_id_end)
		paper_ids = paper.objects.filter(paperset_id__in = papersets).values('id')
		if (topic_id =="All topics"):
                        qnlist = question.objects.filter(paper_id__in=paper_ids).values('id')
                else:                        
                        qnlist = question.objects.filter(paper_id__in=paper_ids,topic_id__in=topics).values('id')
		param['dan']=qnlist
	...
	param['paperset_id_end']=int(paperset_id_end)
	if(topic_id != "All topics"):
                topic_id = int(topic_id)
	param['topic_id']=(topic_id)
	param['topics'] = topic.objects.filter(subject_id = subj_id)
	param['subj_id'] = subj_id
	param['combi'] = combi
	param['tag_type'] = tag_type
...
#summary
def analyzer_summary(request,subj_id):
        
        param={}
        param['cur_subj']=subj_id
	...
	if request.GET.get("topic_id") != None:
        ...
        if (request.GET.get("start_paperset") != None) :
                ...
                paper_ids = paper.objects.filter(paperset_id__in=papersets).values('id')
                questions = question.objects.filter(paper_id__in=paper_ids).values('topic_id')
                topic_number_questions=question.objects.filter(paper_id__in=paper_ids,topic_id=topic_id).values('id')
                param['topic_number_questions']=len(topic_number_questions)
                param['number_questions']= len(questions)
                param['percent']=round(100*float(len(topic_number_questions))/len(questions),2)
                
                #prepare for each topic
                import collections
                
                
                questiondan_raw = question.objects.filter(paper_id__in=paper_ids)
                topicdan=[]
                for tt in questiondan_raw:
                        tt = topic.objects.filter(title=tt.topic_id)
                        topicdan.append((tt[0].id))
                
                counterdan = collections.Counter(topicdan)
                topicdans = counterdan.most_common()
                
                topic_displaydan=[]
                
                param['topicdan']=topicdan
                
                for in1 in topicdans:
                        item={}
                        item_questions=question.objects.filter(paper_id__in=paper_ids,topic_id=in1[0]).values('id')
                        item['freq']=len(item_questions)
                        item['title']=in1
                        a = int(in1[1])
                        topicdada = topic.objects.filter(id__in=[int(in1[0])])
                        item['title']=topicdada[0].title
                        item['id']=topicdada[0].id
                        item['percentage']=round(100*float(len(item_questions))/len(questions),2)
                        topic_displaydan.append(item)
                param['dandan']=topic_displaydan
                

                concepts_raw = tag.objects.filter(question_id__in=topic_number_questions)
                concepts=[]
                for concept in concepts_raw:
                        concepts.append(concept.tag_id)
                ...
                keyword_freq=0
                for t in concepts:
                    concept_tuple = tag_definitions.objects.filter(id__in=[(t[0])])
                    if(concept_tuple[0].type =='C'):
                        concept_freq+=t[1]
                    if(concept_tuple[0].type =='F'):
                        formula_freq+=t[1]
                    if(concept_tuple[0].type =='K'):
                        keyword_freq+=t[1]
                        
                for t in concepts:
                    concept={}
                    concept_tuple = tag_definitions.objects.filter(id__in=[(t[0])])
                    concept['title']=concept_tuple[0].title
                    concept['content']=concept_tuple[0].content
                    concept['id']=concept_tuple[0].id
                    concept['freq']=t[1]
                    
                    if(concept_tuple[0].type =='C'):
                        concept['percentage']=round(100*float(t[1])/concept_freq,2)
                        concept_display.append(concept)
                    if(concept_tuple[0].type =='F'):
                        concept['percentage']=round(100*float(t[1])/formula_freq,2)
                        formula_display.append(concept)
                    if(concept_tuple[0].type =='K'):
                        concept['percentage']=round(100*float(t[1])/keyword_freq,2)
                        keyword_display.append(concept)
                
                
                param['concept_freq']= concept_freq
                param['concept_display']=concept_display
                param['formula_freq']=formula_freq
                param['formula_display']=formula_display
                param['keyword_display']=keyword_display
                param['keyword_freq']=keyword_freq
                
                
##                import collections
##                counter = collections.Counter(questions)
##                topic_counter = counter.most_common()
##                
##                q={}
##                
##                
##                topic_display=[]
##                for t in topic_counter:
##                        topica = {}
##                        topics=[t[0]]# list of topic {actually only one in each list}
##                        topica['title']= topic.objects.filter(id__in=topics).values('title')
##                        topica['percent']=round(100*float(t[1])/float(len(questions)),2)
##                        topica['number_questions']=t[1]
##
##                        concept_display =[]
##                        concepts = tag_definitions.objects.filter(topic_id = t[0],type__in='C')
##                        concept_frequent = tag.objects.filter(tag_id__in=concepts).value('id')
##                        c_counter = collections.Counter(concept_frequent)
##                        concept_counter = c_counter.most_common() #list of concept in a topic with frequency prioritized
##                        topica['number_concept']=len(concepts)
##                        topica['number_concept_match']=len(concept_frequent)
##                        for c in concept_counter:
##                                concept={}
##                                concept['title']=tag_definitions.objects.filter(id__in=c[0])
##                                concept['number_questions']=c[1]
##                                concept['percent']=round(100*float(c[1])/float(t[1]),2)
##                                concept_display.append(concept)#add each concept statistic
##                        topica['concept_display']=concept_display# assign concept display
##                        
##                        topic_display.append(topica)#add each topic statistic
##                q['topic_display']=topic_display
##                param['display']=q
##                param['number_questions'] = len(questions)
        param['subj_id'] = subj_id
        ...
        if request.user.is_authenticated():
            param['user']='logined'
	return render_to_response('analyzer_summary.html',param,RequestContext(request))
...
def analyzer_paper_formula_distribution(request, subj_id):
	...
	if request.GET.get("paperset_id") != None: #compute only if paperset is chosen
		...
		for c in formulas:
			...
			for q in questions:
				tag_result = tag.objects.filter(question_id=q.id, tag_id=c.id)
				if len(tag_result) > 0: #question has formula
					formula_count += 1
				total_count += 1
			if formula_count > 0:
				dataset.append({'name': c.title,
								'value': formula_count})
			formulaData.append({'formula_id': c.id,
								'formula_title': c.title,
								'count': formula_count})
		pset = paperset.objects.get(id=paperset_id)
		sidebarData.append({'paperset_id': pset.id,
							'paperset': pset.title,
...
def analyzer_paper_formula_trend(request, subj_id):
	...
	for pset in papersets:
		...
		for formula in formulas:
			formula_count = len(tag.objects.filter(question_id_id__in=qnlist, tag_id=formula.id))
